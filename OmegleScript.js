...
//alt + q message
var greetText = "It's just something I kill time with every now and then. Usually on the side of some other thing.";
...
function autoGreetMsg(){
  var greetings = ["Hey.","Hello.","Howdy.","Ahoy.","Greetings.","Hi.","Hey there.","Hi there.","Hello there.","Heya.","Aye siwmae."];
        ...
        if (strangerMsgNum <= messagesChecked) {
          ...
          if (disconnectOnNoCaps) {
            if (!grammarIsValid(amsg)) {
              ...
              timedDisconnect();
              msg[msg.length - 1].innerHTML= dye(amsg,"red","grammar"); 
            }
          }
          if (disconnectOnFilter) {
            if (matchedFilter(amsg)) {
              ...
              timedDisconnect();
              msg[msg.length - 1].innerHTML= dye(amsg,"red","filter");
            }
          }
          if (disconnectOnMessage) {
            ...
            for (var j = 0; j < phrasesThatTriggerForth.length; ++j) {
              if (amsg == phrasesThatTriggerForth[j]) {
                ...
                timedDisconnect();
                msg[msg.length - 1].innerHTML= dye(amsg,"red","phrase");
              }
            }
          }
        }
